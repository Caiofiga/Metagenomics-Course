...
#   plot-vcfstats -p freebayes/plots freebayes/patient1.vcf.stats
#
# Edit as necessary and recreate the plots by running
#   python freebayes/plots-plot.py
#
# Title abbreviations:
# 	 0 .. patie .. freebayes/patient1-variants-filtered.vcf
#
...
dat = {}
with open('freebayes/plots-counts_by_af.snps.dat', 'rb') as f:
...
if plot_SNP_count_by_af:
	...
	if 0 in dat and len(dat[0])>2:
		ax1.plot([row[0]/100. for row in dat[0]], [row[1] for row in dat[0]], '-o',markersize=3, color='orange',mec='orange',label='patie')
		has_data = 1
	...
	if has_data:
		...
		plt.savefig('freebayes/plots-counts_by_af.snps.png')
		if pdf_plots: plt.savefig('freebayes/plots-counts_by_af.snps.pdf')
		plt.close()
...
dat = {}
with open('freebayes/plots-counts_by_af.indels.dat', 'rb') as f:
	reader = csv.reader(f, 'tab')
	for row in reader:
		if row[0][0] == '#': continue
		id = int(row[0])
...
if plot_Indel_count_by_af:
	...
	if has_data:
		...
		plt.subplots_adjust(bottom=0.2,left=0.1,right=0.95)
		plt.savefig('freebayes/plots-counts_by_af.indels.png')
		if pdf_plots: plt.savefig('freebayes/plots-counts_by_af.indels.pdf')
		plt.close()
...
if plot_substitutions:
	...
	plt.title('patie')
	plt.savefig('freebayes/plots-substitutions.0.png')
	if pdf_plots: plt.savefig('freebayes/plots-substitutions.0.pdf')
	plt.close()
